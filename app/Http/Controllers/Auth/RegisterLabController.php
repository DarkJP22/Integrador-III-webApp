...
class RegisterLabController extends Controller
{
    /*
    |--------------------------------------------------------------------------
    | Register Controller
    ...
    */
     ...
     * @var string
     */
    protected $redirectTo = '/lab/agenda';
    ...
    {
        $plans = Plan::where('for_lab', 1)->get();

        return view('auth.labs.register', compact('plans'));
    }
    ...
    public function registerAdmin(CreateUser $createUser)
    {
        ...
        $user = DB::transaction(function () use ($createUser) {
            $data = request()->all();
            $user = $createUser($data, Role::where('name', 'laboratorio')->first(), [
                'ide' => ['nullable'],
        ...
        return Redirect('/lab/office/register');
    }
    ...
    protected function create(array $data)
    {
        ...
        $data['active'] = 1; // los labs están inactivos por defecto para revision
        $data['role'] = Role::whereName('laboratorio')->first();
...
//        }
        ...
        return view('auth.labs.registerOffice');
    }
    ...
    public function registerOffice()
    {
        ...
        $data = request()->all();
        $data['type'] = OfficeType::LABORATORY->value;
        ...
        $data['fe'] = 1;
        $data['settings']['lab_whatsapp_message'] = 'Hola deseo ser contactado para asesoría de examenes de laboratorio';
        $data['settings']['lab_whatsapp_exam_message'] = 'Hola deseo ser contactado para asesoría de examenes de laboratorio';
        $data['settings']['lab_whatsapp_package_exam_message'] = 'Hola deseo solicitar una cita para paquete';
        $data['settings']['lab_exam_cash_discount'] = 0;
