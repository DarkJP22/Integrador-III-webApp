...
class SubscriptionController extends Controller
{
    ...
    public function index()
    {
        ...
        $plans = Plan::where('for_clinic', 1)->get(); //centro medico


        return view('clinic.subscriptions.index', compact('plans'));
    }
    ...
    public function renew($id = null)
    {
        ...
        return view('clinic.subscriptions.renew')->with(compact('incomes', 'purchaseOperationNumber', 'amount', 'amountTotal', 'purchaseOperationNumber', 'purchaseCurrencyCode', 'purchaseVerification', 'medic_name', 'medic_email', 'incomesIds', 'description'));
    }
    ...
    public function create(Plan $plan)
    {
        ...
        if (!Plan::where('for_clinic', 1)->get()->contains('id', $plan->id)) return Redirect('/');
        ...
        return view('clinic.subscriptions.buy')->with(compact('newPlan', 'purchaseOperationNumber', 'amount', 'amountTotal', 'purchaseOperationNumber', 'purchaseCurrencyCode', 'purchaseVerification', 'medic_name', 'medic_email', 'description', 'planBuyChange','office'));
    }
    ...
    public function edit(Plan $plan)
    {
        ...
        //     flash('No puedes cambiar de subscripcion hasta que finalices el periodo de la actual', 'danger');
        ...
        return view('clinic.subscriptions.change')->with(compact('newPlan', 'purchaseOperationNumber', 'amount', 'amountTotal', 'purchaseOperationNumber', 'purchaseCurrencyCode', 'purchaseVerification', 'medic_name', 'medic_email', 'description', 'planBuyChange'));
    }
    ...
    public function changeVoucher(Plan $plan)
    {
        ...
        DB::transaction(function () use ($plan) {
            ...
            if ($subscription->plan_id != $plan->id) {
                ...
                if ($income) {
                    $income->description = 'Cambio de plan de subscripcion de '. getDefaultCurrency()?->symbol . $cost_plan_anterior . ' al ' . $plan->title;
                    $income->paid = 1;
                    $income->purchase_operation_number = request('purchase_operation_number');
                    ...
                    auth()->user()->incomes()->create([
                        ...
                        'description' => 'Cambio de plan de subscripcion de '. getDefaultCurrency()?->symbol . $cost_plan_anterior . ' al ' . $plan->title,
                ...
                if (auth()->user()->email) {
                    try {
                        ...
                        auth()->user()->notify(new PaymentSubscriptionConfirmation($plan, request('purchase_operation_number')));
                    } catch (TransportExceptionInterface $e)  //Swift_RfcComplianceException
                    {
                        \Log::error($e->getMessage());
                    }
    ...
    public function changeToFreeSubscription(Plan $plan)
    {
        ...
        if( !$plan->cost > 0 ){ // si es gratis o coste 0
                ...
                if($user->subscription){
                    ...
                    flash('Compra de subscripción realizada correctamente', 'success');
                }
