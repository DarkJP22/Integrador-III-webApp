...
<section class="content-header">
    ...
    @if( ($authorizationResult == 00 || $authorizationResult == "Success") && isset($purchaseOperationNumber) )
        @if(isset($reserved3) && $reserved3 == 1 && $adminCentroMedico)
          <div class="row">
            <div class="col-xs-12">
                ...
                <div class="callout callout-warning">
                  ...
                  <p>Se ha creado un administrador del centro médico con los siguientes datos:<p>
                    <ul>
                      <li>Nombre: {{ $adminCentroMedico->name }}.</li>
                      <li>Correo: {{ $adminCentroMedico->email }}.</li>
                      <li>Contraseña Temporal: {{ $adminCentroMedico->phone_number }}.</li>
                    </ul>   
                  
                  <p> Por favor recuerda cambiar la contraseña, ademas de que debes configurar los parámetros iniciales de la facturación electrónica para funcionar correctamente. Puedes cambiar de perfil y configuralo. <button type="button" class="btn btn-secondary" onclick="event.preventDefault();
                                                      document.getElementById('switch-form-response').submit();">Cambiar a perfil del administrador de centro médico</button></a>
                        <form id="switch-form-response" action="{{ url('/accounts/'.$adminCentroMedico->id.'/switch') }}" method="POST" style="display: none;">
                                          @csrf
                                      </form></p>
                </div>
