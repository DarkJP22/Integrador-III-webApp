...
@section('content')
            ...
            <div class="col-md-4">
                ...
                <div class="box box-solid box-medics">
                    <div class="box-header with-border">
                        <h4 class="box-title">Médicos </h4> <a href="/clinic/agenda/rooms" class="pull-right"> Vista Salas</a>
                        <div><small>(Haz click en un medico para ver su agenda de citas)</small></div>
                        <div>
                            <form action="/clinic/agenda" method="GET">
                                ...
                                @foreach ($medics as $doctor)
                                    <li class="item {{ isset($medic) && $doctor->id == $medic->id ? 'medic-list-selected' : '' }}">
                                        ...
                                        <div class="medic-info">
                                            <a href="/clinic/agenda?medic={{ $doctor->id }}{{ request('page') ? '&page=' . request('page') : '' }}&q={{ isset($q) ? $q : '' }}" class="medic-title">{{ $doctor->name }}
                                            </a>

                                            @if (!$doctor->verifyOffice($office->id))
                                                <button type="submit" class="btn btn-danger btn-xs pull-right" form="form-active-inactive" formaction="/medics/{{ $doctor->id }}/offices/{{ $office->id }}/verify">Pendiente</button>
                                            @else
                                                <a href="/clinic/agenda?medic={{ $doctor->id }}{{ request('page') ? '&page=' . request('page') : '' }}" class="btn btn-primary btn-sm pull-right" title="Ver Calendario"><i class="fa fa-calendar"></i></a>
                                            @endif
                                        ...
                                        <tr>
                                            @foreach ($medicsWithSchedules as $doc)
                                                <td class="calendar-medic-day">
                                                    @php
                                                        $user_settings = $doc->getAllSettings();
                                                    @endphp
    ...
    <modal-clinic-new-appointment :patient="{{ isset($p) ? $p : 'false' }}" :office="{{ $office->id }}" :optreatments="{{ $optreatments }}" :is-esthetic="{{ $office->is_esthetic ?? 'false' }}" endpoint="/clinic/patients">
