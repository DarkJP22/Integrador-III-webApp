...
<x-mail::table>
    |        |        |
    | ------------- | ------------- |
    | <img src="{{ config('app.url').'/img/calendar.png' }}" alt="fecha" style="margin-right: 15px;" /> | Fecha de la cita: <b>{{ $appointment->date->toDateString() }}</b> <br/>  Hora de la cita: <b>{{ Carbon\Carbon::parse($appointment->start)->ToTimeString() }}</b> <br/> Profesional: Dr(a). {{ $appointment->user?->name ?? $appointment->medic_name }}    |
