...
@section('content')
    ...
    @if(isset($newPlan) && $newPlan)
      ...
      <div class="row no-print">
         ...
         <form action="/pharmacy/subscriptions/{{ $newPlan->id }}/change-voucher" method="POST" enctype="multipart/form-data">
          @csrf
          ...
          @endif
          <input class="form-control" type="hidden" name="purchase_operation_number" value="{{ $purchaseOperationNumber }}" />
          @if(isset($income))
            <input class="form-control" type="hidden" name="income" value="{{ $income->id }}" />
          @endif
