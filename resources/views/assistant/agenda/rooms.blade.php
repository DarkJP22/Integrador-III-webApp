        ...
        <div class="col-md-4">
          ...
          <div class="box box-solid box-medics">
            <div class="box-header with-border">
              <h4 class="box-title">Salas </h4> <a href="/assistant/agenda" class="pull-right"> Vista Médicos</a>
              <div><small>(Haz click en una sala para ver su agenda de citas)</small></div>
              <div>
                  <form action="/assistant/agenda/rooms" method="GET">
                ...
                <ul class="medic-list medic-list-in-box">
                  @foreach($rooms as $sala)
                    <li class="item {{ (isset($room) && $sala->id == $room->id) ? 'medic-list-selected': '' }}">
                      ...
                      <div class="room-info">
                        <a href="/assistant/agenda/rooms?room={{$sala->id }}{{ (request('page')) ? '&page='.request('page') : '' }}&q={{ isset($q) ? $q : '' }}" class="medic-title">{{ $sala->name }}
                          </a>
                         
                          
                           
                            <a href="/assistant/agenda/rooms?room={{ $sala->id }}{{ (request('page')) ? '&page='.request('page') : '' }}" class="btn btn-primary btn-sm pull-right" title="Ver Calendario"><i class="fa fa-calendar"></i></a>
