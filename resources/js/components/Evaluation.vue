    ...
    <div class="row">
      ...
      <div class="col-sm-12">
        <div>
          ...
          <div v-show="corp">
            <div>
              <div class="margin" v-for="item in optionsCorporalComputed" :key="item.id">
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" v-model="item.zone" @keydown="keydown(item)" />
                </div>
              </div>
              <div>
                <label for="notes">Notas</label>
          ...
          <div v-show="depi">
            <div class="row">
              <div class="col-sm-6">
                <h4>Opciones:</h4>
                <div class="list-group">
                  <button type="button" class="list-group-item" v-for="item in optionsDepilacionComputed" :key="item.id" :class="{ disabled: item.checked }" @click="selectOption(item)" :disabled="item.checked || loader">
                    <div class="checkbox">
                      <label @click.prevent>
                        <input type="checkbox" :checked="item.checked" :disabled="item.checked" />
                        {{ item.name }}
...
export default {
            ...
            this.optionsFacial.forEach((element) => {
                items.push({
                    ...
                    checked: this.facialCurrentItems.find(
                        (item) => item.opevaluation_id === element.id
                    )
                        ? true
                        : false,
                });
        ...
        optionsCorporalComputed() {
            ...
            this.optionsCorporal.forEach((element) => {
                items.push({
                    ...
                    name: element.name,
                    category: element.category,
                    zone: this.corporalCurrentItems.find(
                        (item) => item.opevaluation_id === element.id
                    )?.zone,
                });
            });
    ...
    methods: {
        next() {
            $('a[href="#anthropometry"]').tab('show');
        },
        showForm() {
            $('#evaluationOptionModal').modal();
        },
        ...
        selectOption(option) {
            const itemFound = _.find(
                ...
                function (o) {
                    return o.opevaluation_id === option.id;
                }
            );
        ...
        saveNotes: _.debounce(function (note) {
            if (!note.id) {
                axios
                    .post(
                        '/appointments/' + this.appointment.id + '/evaluations-notes',
                        note
                    )
                    .then(({ data }) => {
                        ...
                        this.emitter.emit('actSummaryEvaluationNotes', { data: this[`${note.category}Notes`], category: note.category });
                    })
                    .catch(() => {
                        this.loader = false;
        ...
        remove(option) {
            axios
                .delete(
                    '/appointments/' + this.appointment.id + '/evaluations/' + option.id
                )
                .then(() => {
                    this.loader_message = 'Cambios Guardados';
                    this.loader = false;

                    this.emitter.emit('actSummaryEvaluation', { data: this[`${option.category}CurrentItems`], category: option.category });
                })
                .catch(() => {
                    this.loader = false;
                    flash('Error al guardar cambios', 'danger');
        ...
        save(option) {
            ...
            axios
                .post('/appointments/' + this.appointment.id + '/evaluations', {
                    opevaluation_id: option.id,
                    name: option.name,
        ...
        prepareData() {
            if (this.evaluations.length) {
                this.facialCurrentItems = this.evaluations.filter(
                    (eva) => eva.category == 'facial'
                );
                this.corporalCurrentItems = this.evaluations.filter(
                    (eva) => eva.category == 'corporal'
                );
                this.depilacionCurrentItems = this.evaluations.filter(
    ...
    created() {
        console.log('Component ready. evaluation.');
