<template>
  <div class="content">
      ...
      <div class="col-sm-12">
        <!-- <div class="clearfix">
          <div class="pull-right">
            ...
            <modal-option-form
              @created="add"
              url="/beautician/estreatments/options"
              name="treatmentOptionModal"
            ></modal-option-form>
          </div>
        </div> -->
        <div v-show="facial">
    ...
    <treatments-summary :treatments="treatmentsComputed" :appointment="appointment"></treatments-summary>
  </div>
</template>

<script>

import TreatmentsSummary from './TreatmentsSummary.vue';
export default {
    props: {
        read: {
            type: Boolean,
    ...
    computed: {
        treatmentsComputed() {
            const items = this.facialCurrentItems.concat(this.corporalCurrentItems);
            return items.concat(this.depilacionCurrentItems);//.concat(depilacionCurrentItems);
        },
            ...
            this.optionsFacial.forEach((element) => {
                items.push({
                    ...
                    checked: this.facialCurrentItems.find(
                        (item) => item.optreatment_id === element.id
                    )
                        ? true
                        : false,
                });
    ...
    methods: {
        ...
        selectOption(option) {
            const itemFound = _.find(
                ...
                function (o) {
                    return o.optreatment_id === option.id;
                }
            );
        ...
        saveNotes: _.debounce(function (note) {
            if (!note.id) {
                axios
                    .post(
                        '/appointments/' + this.appointment.id + '/estreatments-notes',
                        note
                    )
                    .then(({ data }) => {
                        ...
                        this.loader = false;
                        this.emitter.emit('actSummaryTreatmentNotes', { data: this[`${note.category}Notes`], category: note.category });
                    })
                    .catch(() => {
                        this.loader = false;
        ...
        remove(option) {
            axios
                .delete(
                    '/appointments/' + this.appointment.id + '/estreatments/' + option.id
                )
                .then(() => {
                    ...
                    // this.emitter.emit("actSummaryTreatment", { data: this[`${option.category}CurrentItems`], category: option.category});
                })
                .catch(() => {
                    this.loader = false;
                    flash('Error al guardar cambios', 'danger');
        ...
        save(option) {
            ...
            axios
                .post('/appointments/' + this.appointment.id + '/estreatments', {
                    optreatment_id: option.id,
                    name: option.name,
        ...
        prepareData() {
            if (this.estreatments.length) {
                this.facialCurrentItems = this.estreatments.filter(
                    (eva) => eva.category == 'facial'
                );
                this.corporalCurrentItems = this.estreatments.filter(
                    (eva) => eva.category == 'corporal'
                );
                this.depilacionCurrentItems = this.estreatments.filter(
    ...
    created() {
        console.log('Component ready. treatment.');
        this.prepareData();
    },
};
</script>
