            ...
            <div class="modal-content">
                ...
                <div class="modal-body" data-modaldate data-modaldate-end data-office data-officename>
                    ...
                    <div v-show="!newPatient && !showPackages && !showPendingPayment" class="box-widget widget-user-2"
                         ...
                         v-bind:data-title=" (paciente) ? paciente.first_name : 'Paciente Desconocido' ">
                ...
                <div v-show="!newPatient" class="modal-footer">
                    ...
                    <template v-if="isMedicProfessional">
                        <button v-if="(hasSubscription && !monthlyCharges.length && isPatient) || (isPatient && validated)" class="btn btn-secondary btn-iniciar-cita"
                                type="button">
                            Iniciar consulta
                        </button>
                        <button v-if="!hasSubscription && isPatient" class="btn btn-secondary btn-show-package"
                                type="button" @click="showPackages = true">Iniciar consulta
                        </button>
                        <button v-if="monthlyCharges.length && isPatient" class="btn btn-secondary btn-show-pending-payment"
                                type="button" @click="showPendingPayment = true">
                            Iniciar consulta
                        </button>
                    </template>
...
export default {
    ...
    computed: {
        isMedicProfessional() {
            return window.App.user.type_of_health_professional === 'medico';
        }
    },
    methods: {
        requestCode() {
